...
//	int rescue_alignment(Read const &read, std::string & read_anno, int ori, int &num_A, int &num_T, int &num) ;
	...
	std::vector<SuperVariant> create_super_sequence_from_variants(std::vector<Variant> & variants, std::string & dna, double *&acceptor, int & a_len, double *&donor, int & d_len, int &hit_dna_converted) const ;
	std::vector<FoundVariant> reconstruct_reference_alignment(std::vector<Variant> & variants, std::vector<SuperVariant>& super_variant_list, std::string & dna, int * s_align, int & s_len, int * e_align, int & e_len) const ;
