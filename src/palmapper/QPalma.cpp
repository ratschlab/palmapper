...
template<int myverbosity, bool discover_variants> 
void QPalma::determine_read_variants(Chromosome const &contig_idx, const int * s_align, const int* e_align, const int *dna_align, const int *est_align, const std::vector<int> & positions, 
	...
	if (non_consensus_search)
	{
		alignment.myalign_fast(strand, contig_idx, nr_paths_p, (char*) dna.c_str(), (int) dna.length(), est,
							   ...
							   * alignment_parameters->matchmatrix_dim[1], donor, d_len,
							   ...
							   _config.USE_VARIANTS, _config.NO_GAP_END,_config.SPLICED_MIN_SEGMENT_LENGTH,_config.SPLICED_SHORTEST_INTRON_LENGTH);
	}
	else
		alignment.myalign_fast(strand, contig_idx, nr_paths_p, (char*) dna.c_str(), (int) dna.length(), est,
							   est_len_p, prb, alignment_parameters->h,
