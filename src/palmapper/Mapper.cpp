...
Mapper::Mapper(Genome const &genome_, GenomeMaps &genomemaps_, QueryFile &queryFile, QPalma &qpalma, Reporter &reporter, 
			   JunctionMap &junctionmap, JunctionMap &annotatedjunctions, VariantMap & variants)
	:	GENOME(genome_.LONGEST_CHROMOSOME), _genome(genome_), _genomeMaps(genomemaps_), 
		_junctionmap(junctionmap), _annotatedjunctions(annotatedjunctions), _variants(variants), _queryFile(queryFile), _qpalma(qpalma), _reporter(reporter)
{
	REDUNDANT = 0;
